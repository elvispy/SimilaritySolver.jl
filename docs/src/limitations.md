...
## Additive terms are assumed homogeneous

`_collect_add_terms` splits only *top-level* sums, and the scaling degree of a nested sum is
taken from its first summand. A PDE written as `Dx(u) - (q^2 - H(q)^2)` therefore never
inspects the second summand, whereas the expanded `Dx(u) - q^2 + H(q)^2` does. When a nested
sum is genuinely inhomogeneous the invariance system is silently built from the first term
only. **Expand your equations** so every term appears at the top level.
